...
void serialEvent() {
  while (Serial.available() > 0) {
    ...
    if (incoming_char == '<') { // Check if start character is received
      received_message = "";
    ...
    if (message_started) {
      ...
      if (incoming_char == '>') { // Check if end character is received
...
void process_message(String message) {
  ...
  for (int i = 0; i < message.length() && pos < 10; i++) { // loop through the string
...
String realtime()
{
  ...
  String timeStamp = String(currentYear) + "0" + String(currentMonth) + String(monthDay) + String(currentHour) + String(currentMinute);
  return timeStamp;
}
...
void updateSheets() {
  ...
  delay(5000);    // keep delay of minimum 2 seconds as dht allow reading after 2 seconds interval and also for google sheet
}
